  ...
  if ($password == $cpassword) {
    ...
    if (!$result->num_rows > 0) {
      if (filter_var($email, FILTER_VALIDATE_EMAIL)){
        $sql = "INSERT INTO recruiter (username, email,phone,company,password)
            VALUES ('$username','$email','$phone','$company','$password')";
        $result = mysqli_query($conn, $sql);
        if ($result) {
          header("Location: ../login/login.php");
          die;
    ...
    <div class="container">
        <form method="POST" action=" " id="form" class="form">
            ...
            <div class="form-control">
                ...
                value="<?php echo isset($_POST['username']) ? $_POST['username'] : ''; ?>" pattern="[a-zA-Z. ]{1,50}" required>
