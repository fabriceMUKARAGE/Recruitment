...
class Database
{
   ...
   // private $dsn = "sqlsrv:Server=localhost;Database=test";    // Conect with SQLServer
    ...
    {
        $data = array();
        $sql = "SELECT id, username, email, school, major, year,skills,job, resume FROM approvedstudent WHERE major='Computer Science' order by id DESC";
    ...
    { 
        $sql = "SELECT id, username, email, school, major, year, resume FROM approvedstudent WHERE id=:id";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute(['id' => $id]);
